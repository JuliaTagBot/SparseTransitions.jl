function fast_minim(rcount; max_iter=7, λ=0.01, weighted=true, threshold=10^(-4),
         refit=true,
  ...
  non_jump_days = setdiff(2:t_final, jump_days)
  ...
  @objective(m1, Min,  sum( wts[:,1:4].*(theta_pred_l.-rs[:,1:4]).^2) +
  ...
  @objective(m2, Min,  sum( wts[:,1:4].*(theta_pred_l_c.-rs[:,1:4]).^2) +
  ...
  theta_m_r = getvalue(theta_m_r_c)
   ...
   P2_sparsity = 1 .*(P2 .> threshold)
    ...
    res =   Dict("P1"=>P1, "P2"=>P2,
         "obj_vals"=>obj_vals,
         "theta_m" => [theta_m_l theta_m_r])

    if refit
        post_res = fast_minim(rcount;
                             max_iter=max_iter, λ=0.0,
                             weighted=weighted,
                             threshold=threshold,
                             refit=false,
